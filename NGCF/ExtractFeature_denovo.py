...
if __name__=="__main__":
    ...
    # denovo start
    foldi = 1
    for i in range(num_items): #num_items
        print("#i=",i)

        ############这里应该要改############
        FeaturePath = './Feature/denovo/dataset1_gcn_embedding_128_feature_fold%d.h5' % foldi  # 注意更改

        ###########################更改读文件的方式#######################################
        train_exist_users = []
        train_exist_items = []
        trainfile_path = '../Data/dataset1/denovo/circRNA-disease-fold%d/train.txt' % foldi  # 注意更改
        testfile_path = '../Data/dataset1/denovo/circRNA-disease-fold%d/test.txt' % foldi  # 注意更改
    ...
    with h5py.File('./PlotFigure/IGNSCDA_dataset1_denovo_AUC.h5','w') as hf:
        hf['fpr'] = mean_denovo_fpr
        hf['tpr'] = mean_denovo_tpr
    with h5py.File('./PlotFigure/IGNSCDA_dataset1_denovo_AUPR.h5','w') as h:
    ...
    plt.legend(loc=0)
    plt.savefig("./FinalResultPng/roc-IGNSCDA_dataset1_denovo.png")
