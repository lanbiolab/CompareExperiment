...
if __name__=="__main__":
    ...
    # denovo start
    # foldi = 1
    for i in range(90,104): #num_items
        print("#i=",i)

        ############这里应该要改############
        FeaturePath = '../Feature/denovo/dataset4_gcn_embedding_128_feature_fold%d.h5' % (i+1)  # 注意更改

        ###########################更改读文件的方式#######################################
        train_exist_users = []
        train_exist_items = []
        trainfile_path = '../../Data/dataset4/denovo/circRNA-disease-fold%d/train.txt' % (i+1)  # 注意更改
        testfile_path = '../../Data/dataset4/denovo/circRNA-disease-fold%d/test.txt' % (i+1)  # 注意更改
    ...
    # 用h5py数据形式将它存储下来
    with h5py.File('./IGNSCDA_dataset4_denovo_result_90_104.h5', 'w') as hf:
        hf['tpr_arr'] = tpr_arr
        hf['fpr_arr'] = fpr_arr
