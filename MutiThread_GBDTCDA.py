...
@Description: 由于仅仅依赖单线程这个程序跑不快这里我们使用多线程
充分利用服务器的资源
...
if __name__ == '__main__':
    ...
    i=0
    # 开始塞入多线程函数
    fold1_t = MyThread(fold_prosess, args=(circrna_disease_matrix, one_list, i, split))
    fold2_t = MyThread(fold_prosess, args=(circrna_disease_matrix, one_list, i+split, split))
    fold3_t = MyThread(fold_prosess, args=(circrna_disease_matrix, one_list, i+2*split, split))
    fold4_t = MyThread(fold_prosess, args=(circrna_disease_matrix, one_list, i+3*split, split))
    fold5_t = MyThread(fold_prosess, args=(circrna_disease_matrix, one_list, i+4*split, split))
    ...
    with h5py.File('./PlotFigure/GBDTCDA_AUC.h5') as hf:
        hf['fpr'] = mean_cross_fpr
        hf['tpr'] = mean_cross_tpr
    with h5py.File('./PlotFigure/GBDTCDA_AUPR.h5') as h:
    ...
    plt.legend(loc=0)
    plt.savefig("./FinalResultPng/roc-GBDTCDA.png")
