...
if __name__ == '__main__':
    ...
    for i in range(0, len(one_list), split):
        ...
        for i in range(10):
            results.append(pool.apply_async(batch_process,
                                            args=(i * batch_split, i * batch_split + batch_split, rel_matrix, circ_sim_matrix, dis_sim_matrix)))
        ...
        #     input_X_label.append(locals()['batch_' + str(i + 1) + '_input_X_label'])

        for i in range(10):
            input_X += locals()['batch_' + str(i + 1) + '_input_X']
            test_X += locals()['batch_' + str(i + 1) + '_test_X']
            input_X_label += locals()['batch_' + str(i + 1) + '_input_X_label']
        ...
        for i in range(prediction_matrix.shape[0]):
            for j in range(prediction_matrix.shape[1]):
                prediction_matrix[i, j] = prediction[rel_matrix.shape[1] * i + j]
    ...
    with h5py.File('./PlotFigure/GBDTCDA_10fold_AUC.h5') as hf:
        hf['fpr'] = mean_cross_fpr
        hf['tpr'] = mean_cross_tpr
    with h5py.File('./PlotFigure/GBDTCDA_10fold_AUPR.h5') as h:
    ...
    plt.legend(loc=0)
    plt.savefig("roc-GBDTCDA_10fold.png")
