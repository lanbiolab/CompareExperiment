...
with h5py.File('../PlotFigure/LLCDC_AUC.h5') as f:
    LLCDC_tpr = f['tpr'][:]
    LLCDC_fpr = f['fpr'][:]
with h5py.File('../PlotFigure/iCircDA_MF_AUC.h5') as f:
    ICIRCDA_MF_tpr = f['tpr'][:]
    ICIRCDA_MF_fpr = f['fpr'][:]
with h5py.File('../PlotFigure/RWR-KNN_AUC.h5') as f:
    RWR_KNN_tpr = f['tpr'][:]
    RWR_KNN_fpr = f['fpr'][:]
...
with h5py.File('../PlotFigure/iCircDA_MF_AUPR.h5') as hf:
    ICIRCDA_MF_recall = hf['recall'][:]
    ICIRCDA_MF_precision = hf['precision'][:]
with h5py.File('../PlotFigure/RWR-KNN_AUPR.h5') as hf:
    RWR_KNN_recall = hf['recall'][:]
    RWR_KNN_precision = hf['precision'][:]
with h5py.File('../PlotFigure/GBDTCDA_AUPR.h5') as hf:
...
plt.plot(KATZCPDA_fpr, KATZCPDA_tpr, 'red', label='KATZCPDA AUC=%0.4f'% KATZCPDA_auc) #0.8137
plt.plot(RWR_fpr, RWR_tpr, 'dodgerblue', label='RWR AUC=%0.4f'% RWR_auc) #0.8045
plt.plot(KATZHCDA_fpr, KATZHCDA_tpr, 'darkorange', label='KATZHCDA AUC=%0.4f'% KATZHCDA_auc) #0.7889
plt.plot(DWNN_RLS_fpr, DWNN_RLS_tpr, 'y', label='DWNN_RLS AUC=%0.4f'% DWNN_RLS_auc) #0.7768
plt.plot(CD_LNLP_fpr, CD_LNLP_tpr, 'hotpink', label='CD_LNLP AUC=%0.4f'% CD_LNLP_auc) #0.7693
plt.plot(IBNPKATZ_fpr, IBNPKATZ_tpr, 'g', label='IBNPKATZ AUC=%0.4f'% IBNPKATZ_auc)  #0.5866
plt.plot(LLCDC_fpr, LLCDC_tpr, 'blueviolet', label='LLCDC AUC=%0.4f'% LLCDC_auc) #0.5809
plt.plot(ICIRCDA_MF_fpr, ICIRCDA_MF_tpr, 'cyan', label='ICIRCDA_MF AUC=%0.4f'% ICIRCDA_MF_auc) #0.5633
plt.plot(BRWSP_fpr, BRWSP_tpr, 'saddlebrown', label='BRWSP AUC=%0.4f'% BRWSP_auc) #0.5301
plt.plot(RWR_KNN_fpr, RWR_KNN_tpr, 'cornflowerblue', label='RWR-KNN AUC=%0.4f'% RWR_KNN_auc) #0.4935
plt.plot(GBDTCDA_fpr, GBDTCDA_tpr, 'peru', label='GBDTCDA AUC=%0.4f'% GBDTCDA_auc)
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.legend(loc=0, fontsize=8)
plt.savefig("../FinalResultPng/Figure AUC of all models dataset1.png")
plt.show()

plt.plot(KATZHCDA_recall, KATZHCDA_precision,'darkorange', label='KATZHCDA AUPR=%0.4f' % KATZHCDA_AUPR) #0.0098
plt.plot(RWR_recall, RWR_precision,'dodgerblue', label='RWR AUPR=%0.4f' % RWR_AUPR) #0.0086
plt.plot(KATZCPDA_recall, KATZCPDA_precision,'red', label='KATZCPDA AUPR=%0.4f' % KATZCPDA_AUPR) #0.0073
plt.plot(CD_LNLP_recall, CD_LNLP_precision,'hotpink', label='CD_LNLP AUPR=%0.4f' % CD_LNLP_AUPR) #0.0072
plt.plot(DWNN_RLS_recall, DWNN_RLS_precision,'y', label='DWNN_RLS AUPR=%0.4f' % DWNN_RLS_AUPR) #0.0060
plt.plot(LLCDC_recall, LLCDC_precision,'blueviolet', label='LLCDC AUPR=%0.4f' % LLCDC_AUPR) #0.0033
plt.plot(IBNPKATZ_recall, IBNPKATZ_precision,'g', label='IBNPKATZ AUPR=%0.4f' % IBNPKATZ_AUPR) #0.0032
plt.plot(BRWSP_recall, BRWSP_precision,'saddlebrown', label='BRWSP AUPR=%0.4f' % BRWSP_AUPR) #0.0031
plt.plot(ICIRCDA_MF_recall, ICIRCDA_MF_precision,'cyan', label='ICIRCDA_MF AUPR=%0.4f' % ICIRCDA_MF_AUPR) #0.0031
plt.plot(RWR_KNN_recall, RWR_KNN_precision,'cornflowerblue', label='RWR-KNN AUPR=%0.4f' % RWR_KNN_AUPR) #0.0025
plt.plot(GBDTCDA_recall, GBDTCDA_precision, 'peru', label='GBDTCDA AUC=%0.4f'% GBDTCDA_AUPR) #0.0020
plt.xlabel('Recall')
plt.ylabel('precision')
plt.legend(loc=1, fontsize = 8)
plt.savefig("../FinalResultPng/Figure AUPR of all models dataset1.png")
plt.show()
