...
def prepare_data(fold, seperate=False):
    ...
    circRNA_fea = np.loadtxt("integrated_circRNA_similarity3.txt", dtype=float, delimiter="\t")
    disease_fea = np.loadtxt("integrated_disease_similarity3.txt", dtype=float, delimiter="\t")
    # interaction = np.loadtxt("associations.txt", dtype=float, delimiter=",")
    df = pd.read_excel('associations3.xls', 'sheet1', header=None)
...
def DNN_auto(x_train):
    ...
    decoded = Dense(250, activation='relu')(decoded)
    decoded = Dense(351, activation='tanh')(decoded)
    ...
    autoencoder.fit(x_train, x_train, epochs=1, batch_size=20,shuffle=True)  # second x_train is given instead of train labels in DNN, ie here, i/p=o/p
...
def DeepCDA():
    ...
    df = pd.read_excel('associations3.xls', 'sheet1', header=None)
    interaction = np.array(df)
    for fold in range(interaction.shape[1]+1):
        if fold==interaction.shape[1]:
            break
        print("##################")
        print(fold)
        print("##################")
        X, labels, label, T, inter, link_number = prepare_data(fold,
            seperate=True)  # X= array of concatinated features,labels=corresponding labels
        # import pdb            #python debugger
        # df = pd.read_excel('associations1.xls', 'sheet1', header=None)
        # interaction = np.array(df)
    ...
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations3_denovo_auc.h5') as hf:
        hf['fpr'] = mean_cross_fpr
        hf['tpr'] = mean_cross_tpr
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations3_denovo_aupr.h5') as h:
