...
def prepare_data(seperate=False):
    ...
    circRNA_fea = np.loadtxt("integrated_circRNA_similarity1.txt", dtype=float, delimiter="\t")
    disease_fea = np.loadtxt("integrated_disease_similarity1.txt", dtype=float, delimiter="\t")
...
def DNN_auto(x_train):
    ...
    decoded = Dense(250, activation='relu')(decoded)
    decoded = Dense(622, activation='tanh')(decoded)
...
def DeepCDA():
    X, labels, label, T, interaction, link_number = prepare_data(seperate=True)  # X= array of concatinated features,labels=corresponding labels
    ...
    # with h5py.File(r'E:\pythonProject\KHGAT\data\associations3_5_auc.h5') as hf:
    #     hf['fpr'] = mean_cross_fpr
    #     hf['tpr'] = mean_cross_tpr
    # with h5py.File(r'E:\pythonProject\KHGAT\data\associations3_5_aupr.h5') as h:
    #     h['recall'] = mean_cross_recall
    #     h['precision'] = mean_cross_precision
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations2_10_auc.h5') as hf:
        hf['fpr'] = mean_cross_fpr
        hf['tpr'] = mean_cross_tpr
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations2_10_aupr.h5') as h:
        h['recall'] = mean_cross_recall
        h['precision'] = mean_cross_precision
