...
def prepare_data(fold, seperate=False):
    ...
    circRNA_fea = np.loadtxt("integrated_circRNA_similarity4.txt", dtype=float, delimiter="\t")
    disease_fea = np.loadtxt("integrated_disease_similarity4.txt", dtype=float, delimiter="\t")
    ...
    df = pd.read_excel('associations4.xls', 'sheet1', header=None)
...
def DeepCDA():
    ...
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations4_denovo_auc.h5') as hf:
        hf['fpr'] = mean_cross_fpr
        hf['tpr'] = mean_cross_tpr
    with h5py.File(r'E:\pythonProject\KHGAT\data\associations4_denovo_aupr.h5') as h:
