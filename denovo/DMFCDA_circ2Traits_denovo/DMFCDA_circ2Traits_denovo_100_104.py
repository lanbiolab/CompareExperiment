...
def DMFCDA(init):
    ...
    disease_input = tf.keras.Input(shape=(923,), name='disease_input')
    circRNA_input = tf.keras.Input(shape=(104,), name="circRNA_input")

    left = layers.Dense(923, name="left_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(disease_input)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(462, name="left_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(231, name="left_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)

    right = layers.Dense(104, name="right_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(circRNA_input)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(52, name="right_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(26, name="right_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)

    x = layers.concatenate([left, right], axis=1)

    final_vector = layers.Dense(257, name="final_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(x)
...
if __name__ == '__main__':
    ...
    # 用h5py数据形式将它存储下来
    with h5py.File('./results/DMFCDA_denovo_result_100_104.h5', 'w') as hf:
        hf['tpr_arr'] = tpr_arr
        hf['fpr_arr'] = fpr_arr
