...
def DMFCDA(init):
    ...
    disease_input = tf.keras.Input(shape=(1265,), name='disease_input')
    circRNA_input = tf.keras.Input(shape=(151,), name="circRNA_input")

    left = layers.Dense(1265, name="left_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(disease_input)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(632, name="left_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(316, name="left_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)

    right = layers.Dense(151, name="right_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(circRNA_input)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(75, name="right_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(37, name="right_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)

    x = layers.concatenate([left, right], axis=1)

    final_vector = layers.Dense(353, name="final_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(x)
...
if __name__ == '__main__':
    ...
    # denovo startDMFCDA_circad_denovo_135_140.py
    ...
    # 用h5py数据形式将它存储下来
    with h5py.File('./results/DMFCDA_denovo_result_50_55.h5', 'w') as hf:
        hf['tpr_arr'] = tpr_arr
        hf['fpr_arr'] = fpr_arr
