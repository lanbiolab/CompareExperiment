...
def DMFCDA(init):
    ...
    disease_input = tf.keras.Input(shape=(330,), name='disease_input')
    circRNA_input = tf.keras.Input(shape=(79,), name="circRNA_input")

    left = layers.Dense(330, name="left_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(disease_input)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(165, name="left_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)
    left = layers.Dropout(0.005)(left)
    left = layers.Dense(82, name="left_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(left)

    right = layers.Dense(79, name="right_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(circRNA_input)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(40, name="right_dense_2", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)
    right = layers.Dropout(0.005)(right)
    right = layers.Dense(20, name="right_dense_3", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(right)

    x = layers.concatenate([left, right], axis=1)

    final_vector = layers.Dense(102, name="final_dense_1", activation="relu", kernel_initializer=init, bias_initializer=init, bias_regularizer=regularizers.l2(0.01))(x)
...
if __name__ == '__main__':
    ...
    for i in range(0, len(one_list), split):
        ...
        sorted_circrna_disease_matrix, sorted_score_matrix, sort_index = sortscore.sort_matrix(prediction_matrix,
        ...
        with open('./Data/circRNA_disease_noncancer/circ_list.csv', 'r') as hs:
            circRNA_list = []
            reader = csv.reader(hs)
            for row in reader:
                circRNA_list.append(row[0].lower())
        with open('./Data/circRNA_disease_noncancer/dis_list.csv', 'r') as rs:
            cancer_list = []
        ...
        for num in top_list:
            ...
            for tmp in tmp_list:
                with open(
                        './Data/circRNA_disease_noncancer/circRNA_disease_fold{}/DMFCDA_dataset2_fold{}_top{}.txt'.format(fold, fold,
                                                                                                             num),
                        'a') as fs:
